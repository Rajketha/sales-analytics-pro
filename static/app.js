...
// Dynamic palette resolution (lazy evaluation for light/dark mode)
const C = () => ({
    blue: getVar('--blue'), green: getVar('--green'), red: getVar('--red'),
    orange: getVar('--orange'), purple: getVar('--purple'), teal: getVar('--teal'),
    muted: getVar('--muted'), surf: getVar('--surface'), border: getVar('--border')
});
...
const GRID_OPT = { color: 'rgba(150, 150, 150, 0.1)', tickColor: 'transparent' };
...
async function fetchCharts() {
    ...
    // Resolve palette arrays at runtime so they catch the active theme
    const activePal = [C().blue, C().green, C().purple, C().orange, C().teal, C().red];

    charts['chartRegions'] = new Chart($('chartRegions'), {
        type: 'doughnut',
        data: {
            labels: regions.map(r => r.Region),
            datasets: [{ data: regions.map(r => r.revenue), backgroundColor: activePal, borderColor: C().surf, borderWidth: 2, hoverOffset: 6 }]
        },
        options: {
...
async function fetchOrders() {
    ...
    tbody.innerHTML = orders.map(o => {
        ...
        return `<tr>
          ...
          <td class="hide-sm"><span class="tag tag-${reg}">${o.Region}</span></td>
          <td>${o.Product}</td>
          <td class="hide-sm" style="text-align:center">${o.Quantity}</td>
          <td>${fmt(o.Sales_Amount)}</td>
          <td style="color:${col};font-weight:600">${fmt(o.Profit)}</td>
          <td class="hide-md">${((o.Discount ?? 0) * 100).toFixed(0)}%</td>
          <td class="hide-lg">${o.Gender || '—'}</td>
          <td class="hide-lg">${fmt(o.Annual_Income)}</td>
          <td class="hide-md">${o.Category}</td>
          <td style="text-align:right"><button class="btn btn-danger" onclick="deleteOrder(${o.id})">✕</button></td>
        </tr>`;
    }).join('');
}
...
$('csv-input')?.addEventListener('change', async (e) => {
    ...
    const isClear = $('clear-data-chk')?.checked || false;
    ...
    try {
        ...
        if (!res.ok) {
            ...
            closeModal();
            if (data.insights) await refreshAll(data.insights);
            else await refreshAll();
        }

    } catch (err) {
        toast('❌ Import error (check console)', 'error');
        console.error("Import error:", err);
    } finally {
        if (label) label.textContent = originalText;
        inputElement.value = ''; // Reset so the same file can be selected again
    }
});
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Listen for OS theme changes to quickly adjust chart colors
